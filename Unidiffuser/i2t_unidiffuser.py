...
def prepare_contexts(image, config, clip_text_model, clip_img_model, clip_img_model_preprocess, autoencoder):
...
def evaluate(config):
    ...
    for folder in image_folders:
        ...
        for image in entries:
            ...
            contexts, img_contexts, clip_imgs = prepare_contexts(image, config, clip_text_model, clip_img_model, clip_img_model_preprocess, autoencoder)
            ...
            if config.mode in ['joint']:
                ...
                # os.makedirs(os.path.join(config.output_path, config.mode), exist_ok=True)
                print(os.path.join(config.output_path, config.mode, f'{config.mode}.txt'))
                path = "/home/pxieaf/AttackVLM/stable-diffusion/unidiffuser/img_caption/scienceqa.txt"
...
flags.DEFINE_integer("n_samples", 1000, "the number of samples to generate")
flags.DEFINE_integer("nrow", 4, "number of images displayed in each row of the grid")
flags.DEFINE_string("mode", "i2t",
...
def main(argv):
    ...
    config.directory = FLAGS.directory
